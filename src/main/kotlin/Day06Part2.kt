class Day06Part2 {
    fun solve(input: IntArray): Int {
        val seen = linkedSetOf(input.toList().toString())
    ...
    private tailrec fun solve(stack: Int, index: Int, memory: IntArray, seen: LinkedHashSet<String>): Int {
        val updatedMemory = reallocate(stack, index + 1, memory)
        val added = seen.add(updatedMemory.toList().toString())
        return when (added) {
            false -> seen.size - seen.indexOf(updatedMemory.toList().toString())
            true -> {
